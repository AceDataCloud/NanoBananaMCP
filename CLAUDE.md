...
MCP (Model Context Protocol) server for NanoBanana (Gemini-based) AI image generation via AceDataCloud API.
...
core/
  ...
  types.py      — Literal types (NanoBananaModel, etc.)
  exceptions.py — Error classes (AuthError, APIError, TimeoutError)
  utils.py      — Formatting helpers
tools/
  image_tools.py   — generate and edit images
...
1. **Source of truth** — `_docs/openapi/nanobanana.json` is the OpenAPI spec for the NanoBananaMCP API.
