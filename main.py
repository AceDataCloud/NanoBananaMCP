#!/usr/bin/env python3
"""
MCP NanoBanana Server - AI Image Generation via AceDataCloud API.

A Model Context Protocol (MCP) server that provides tools for generating
and editing AI images using Google's Nano Banana model through the AceDataCloud platform.
"""
...
def safe_print(text: str) -> None:
    ...
    if not sys.stderr.isatty():
        logger.debug(f"[MCP NanoBanana] {text}")
        return
...
def get_version() -> str:
    ...
    try:
        return metadata.version("mcp-nanobanana-pro")
    except metadata.PackageNotFoundError:
        return "dev"


def main() -> None:
    """Run the MCP NanoBanana server."""
    parser = argparse.ArgumentParser(
        description="MCP NanoBanana Server - AI Image Generation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  mcp-nanobanana-pro                    # Run with stdio transport (default)
  mcp-nanobanana-pro --transport http   # Run with HTTP transport
  mcp-nanobanana-pro --version          # Show version
...
Environment Variables:
  ...
  LOG_LEVEL                      Logging level (default: INFO)
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"mcp-nanobanana-pro {get_version()}",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    safe_print("")
    safe_print("=" * 50)
    safe_print("  MCP NanoBanana Server - AI Image Generation")
    safe_print("=" * 50)
    safe_print("")
    ...
    safe_print("    - nanobanana_generate_image")
    safe_print("    - nanobanana_edit_image")
    safe_print("    - nanobanana_get_task")
    safe_print("    - nanobanana_get_tasks_batch")
    safe_print("")
    safe_print("  Available prompts:")
    safe_print("    - nanobanana_image_generation_guide")
    safe_print("    - nanobanana_prompt_writing_guide")
    safe_print("    - nanobanana_workflow_examples")
    safe_print("")
    safe_print("=" * 50)
    ...
    try:
        if args.transport == "http":
            ...
            async def favicon(_request: Request) -> RedirectResponse:
                return RedirectResponse("https://cdn.acedata.cloud/859plc.jpg", status_code=301)
